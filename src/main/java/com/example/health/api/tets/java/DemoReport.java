package com.example.health.api.tets.java;
...
public class DemoReport {
    ...
    public static void createWelcome(XWPFDocument doc, HashMap param) {
        ...
        run.setFontSize(10);
        run.setFontFamily("Microsoft YaHei");
        run.setText("感谢您到昆明市华夏健康中心体检。现将您的体检结果汇总如下，请您认真阅读体检结果和建议。如有疑问，请您来院或者致电本中心服务电话010-6312585，我们将安排专业人员为您答疑解惑。欢迎对我们的工作提出批评和建议。祝您健康！");
        ctp = paragraph.getCTP();
        ctpPr = ctp.addNewPPr();
        ctSpacing = ctpPr.addNewSpacing();
        ctSpacing.setLineRule(STLineSpacingRule.EXACT);
...
//        createCheckupResultByTemplate_2(doc, param);
        ...
        try (FileOutputStream out = new FileOutputStream("D:/word2008.docx");) {
            doc.write(out);
